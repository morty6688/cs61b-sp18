...
public class Connector {
    ...
    public static void addConnectableDirection(List<Connector> l, TETile[][] world, TETile t,
        ...
        switch (d) {
            case RIGHT:
                if (p.getX() + 2 < width && world[p.getX() + 2][p.getY()] == t) {
                    Connector conn = new Connector(new Position(p.getX() + 2, p.getY()), d);
                    l.add(conn);
                }
                break;
            case LEFT:
                if (p.getX() - 2 >= 0 && world[p.getX() - 2][p.getY()] == t) {
                    Connector conn = new Connector(new Position(p.getX() - 2, p.getY()), d);
                    l.add(conn);
                }
                break;
            case UP:
                if (p.getY() + 2 < height && world[p.getX()][p.getY() + 2] == t) {
                    Connector conn = new Connector(new Position(p.getX(), p.getY() + 2), d);
                    l.add(conn);
                }
                break;
            default:
                if (p.getY() - 2 >= 0 && world[p.getX()][p.getY() - 2] == t) {
                    Connector conn = new Connector(new Position(p.getX(), p.getY() - 2), d);
                    l.add(conn);
                }
                break;
